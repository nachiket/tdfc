...
#include "_copy_x2.h"
using namespace std;
char * _copy_x2_name="_copy_x2";
void * _copy_x2_proc_run(void *obj) {
  return(((_copy_x2 *)obj)->proc_run()); }
ScoreOperatorElement *_copy_x2init_instances() {
  return(ScoreOperator::addOperator(_copy_x2_name,0,3,0));  }
ScoreOperatorElement *_copy_x2::instances=_copy_x2init_instances();

_copy_x2::_copy_x2(DOUBLE_SCORE_STREAM n_cc_i,DOUBLE_SCORE_STREAM n_cc_o1,DOUBLE_SCORE_STREAM n_cc_o2)
{
  int *params=(int *)malloc(0*sizeof(int));
  char * name=mangle(_copy_x2_name,0,params);
  ...
  if (instance_fn!=(char *)NULL) {
    ...
    if (res==-1) {
       cerr <<"_copy_x2 msgsnd call failed with errno=" << errno << endl;
       exit(2);    }
  }
  else {
    declareIO(1,2, _copy_x2_name);
  addInstance(instances,this,params);
    ...
    pthread_create(&rpt,a_thread_attribute,&_copy_x2_proc_run, this);
  }
}

void *_copy_x2::proc_run() {
	sleep(1);
...
void _copy_x2::get_graphviz_strings() {
...
#undef NEW__copy_x2
extern "C" void *NEW__copy_x2(DOUBLE_SCORE_STREAM i0,DOUBLE_SCORE_STREAM i1,DOUBLE_SCORE_STREAM i2) {
  return((void *) (new _copy_x2( i0, i1, i2)));
}
