...
#include <stdio.h>
#include <cuda.h>
#include "tdfc_rot.cuh"

int main(void)
{
  const float cc_c_d = (float)1;
  const float cc_s_d = (float)1;
  float *cc_x_h, *cc_x_d;
  float *cc_y_h, *cc_y_d;
  ...
  tdfc_rot <<< n_blocks, block_size >>> (cc_c_d, cc_s_d, cc_x_d, cc_y_d, cc_x_out_d, cc_y_out_d, N);
  ...
  printf("GPU time for tdfc_rot is %f ms\n", time);
#endif
