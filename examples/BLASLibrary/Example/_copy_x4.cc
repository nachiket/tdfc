...
#include "_copy_x4.h"
using namespace std;
char * _copy_x4_name="_copy_x4";
void * _copy_x4_proc_run(void *obj) {
  return(((_copy_x4 *)obj)->proc_run()); }
ScoreOperatorElement *_copy_x4init_instances() {
  return(ScoreOperator::addOperator(_copy_x4_name,0,5,0));  }
ScoreOperatorElement *_copy_x4::instances=_copy_x4init_instances();

_copy_x4::_copy_x4(DOUBLE_SCORE_STREAM n_cc_i,DOUBLE_SCORE_STREAM n_cc_o1,DOUBLE_SCORE_STREAM n_cc_o2,DOUBLE_SCORE_STREAM n_cc_o3,DOUBLE_SCORE_STREAM n_cc_o4)
{
  int *params=(int *)malloc(0*sizeof(int));
  char * name=mangle(_copy_x4_name,0,params);
  ...
  if (instance_fn!=(char *)NULL) {
    ...
    _copy_x4_arg *data;
    struct msgbuf *msgp;
    data=(_copy_x4_arg *)malloc(sizeof(_copy_x4_arg));
    ...
    data->i4=STREAM_OBJ_TO_ID(n_cc_o4);
    alen=sizeof(_copy_x4_arg);
    ...
    if (res==-1) {
       cerr <<"_copy_x4 msgsnd call failed with errno=" << errno << endl;
       exit(2);    }
  }
  else {
    declareIO(1,4, _copy_x4_name);
  addInstance(instances,this,params);
    ...
    bindOutput(3,n_cc_o4,new ScoreStreamType(0,0,1,64));
    out[3]->setName("cc_o4");
    SCORE_MARKWRITESTREAM(n_cc_o4,globalCounter->threadCounter);
    ...
    pthread_create(&rpt,a_thread_attribute,&_copy_x4_proc_run, this);
  }
}

void *_copy_x4::proc_run() {
	sleep(1);
  ...
  while (!done) {
        ...
        if (1&&!eos_0&&!eofr_0) {
          ...
          STREAM_WRITE_DOUBLE(out[3],cc_i_retime[0]);
          state=STATE_only;
        }
        else
        {
...
void _copy_x4::get_graphviz_strings() {
	...
	if(out[3]->src!=NULL && out[3]->sink!=NULL) {
		*(ScoreOperator::fout) << "\t" << out[3]->src->getName() << "->" << out[3]->sink->getName() << "[ label= \" cc_o4\" ]" << endl;
		cout << "\t" << out[3]->src->getName() << "->" << out[3]->sink->getName() << "[ label= \" cc_o4\" ]" << endl;
	} else {cout<<"cannot connect cc_o4 "<<endl;}
	funlockfile(stdout);
}

#undef NEW__copy_x4
extern "C" void *NEW__copy_x4(DOUBLE_SCORE_STREAM i0,DOUBLE_SCORE_STREAM i1,DOUBLE_SCORE_STREAM i2,DOUBLE_SCORE_STREAM i3,DOUBLE_SCORE_STREAM i4) {
  return((void *) (new _copy_x4( i0, i1, i2, i3, i4)));
}
