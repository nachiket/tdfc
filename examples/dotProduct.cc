...
using namespace std;
char * dotProduct_name="dotProduct";
void * dotProduct_proc_run(void *obj) {
  return(((dotProduct *)obj)->proc_run()); }
ScoreOperatorElement *dotProductinit_instances() {
  return(ScoreOperator::addOperator(dotProduct_name,0,3,0));  }
ScoreOperatorElement *dotProduct::instances=dotProductinit_instances();

dotProduct::dotProduct(DOUBLE_SCORE_STREAM n_cc_x,DOUBLE_SCORE_STREAM n_cc_y,DOUBLE_SCORE_STREAM n_cc_z)
{
  int *params=(int *)malloc(0*sizeof(int));
  char * name=mangle(dotProduct_name,0,params);
  ...
  if (instance_fn!=(char *)NULL) {
    ...
    if (res==-1) {
       cerr <<"dotProduct msgsnd call failed with errno=" << errno << endl;
       exit(2);    }
  }
  else {
    declareIO(2,1, dotProduct_name);
  addInstance(instances,this,params);
    ...
    pthread_create(&rpt,a_thread_attribute,&dotProduct_proc_run, this);
  }
}

void *dotProduct::proc_run() {
	sleep(1);
...
void dotProduct::get_graphviz_strings() {
...
#undef NEW_dotProduct
extern "C" void *NEW_dotProduct(DOUBLE_SCORE_STREAM i0,DOUBLE_SCORE_STREAM i1,DOUBLE_SCORE_STREAM i2) {
  return((void *) (new dotProduct( i0, i1, i2)));
}
