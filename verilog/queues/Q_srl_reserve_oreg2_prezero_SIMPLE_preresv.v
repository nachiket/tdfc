// Q_srl_reserve_oreg2_prezero_SIMPLE_preresv.v
//
...
//  - Queue almost-emptiness and reserve mode are pre-computed for next cycle
...
// assign i_b = addr_full_;			// - input bp if full
   assign i_b = (reserve==0) ? addr_full_ : addr_resv;
   ...
   always @* begin					// - combi always
	...
	state_one: begin		    // - (contains one)
		 ...
		 shift_en_   <= 1;
		 addr_       <= 0;
		 addr_resv_  <= (reserve==depth-1) ? 1 : addr_resv;
		 state_      <= state_one;
	      end
...
//		 state_      <= state_more;
		 ...
		 addr_resv_  <= (reserve==depth-1) ? 1 :
				addr_zero ? 0          :
				(addr>=depth-2-reserve);	// (a-1>=d-2-r)
		 ...
		 addr_       <= addr+1;
		 addr_resv_  <= (reserve==depth-1) ? 1 :
				(addr >= ((depth>=3+reserve) ?
					  (depth -3-reserve) : 0));
			     // (a+1>=d-2-r)       (a>=d-3-r)
			     // (a>=max{d-3-r,0})  (a>=((d-3-r>=0)?(d-3-r):(0)))
		 state_      <= state_more;
	      end
		 ...
		 addr_       <= addr_zero ? 0          : addr-1;
		 addr_resv_  <= (reserve==depth-1) ? 1 :
				addr_zero ? 0 : (addr>=depth-1-reserve);
			     //                  (a-1>=d-2-r)
		 state_      <= addr_zero ? state_one  : state_more;
	      end
